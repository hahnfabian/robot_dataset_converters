...
This time the action space is eef positions.
...
def read_episode(input_path, orientation_format="euler_xyz"):
    with h5py.File(input_path, "r") as f:
        ...
        actions = f["puppet/joint_position"][:].astype(np.float32)
...
def align_time(ep):
    # original data
    actions = ep["actions"]              # (T, D)
    obs = ep["obs"]                     # each (T, ...)

    # shift actions: action[t] = action leading to next state
    actions = actions[1:]               # (T-1, D)

    # build new obs and next_obs
    new_obs = {}
    next_obs = {}
    ...
    # trim rewards/dones/absolute actions
    ep["rewards"] = ep["rewards"][:-1]
    ep["dones"] = ep["dones"][:-1]
    ep["absolute_actions"] = ep["absolute_actions"][:-1]

    return ep


def convert_to_delta_actions(ep):
    q_t = ep["obs"]["robot0_joint_pos"]   # (T-1, D)
    q_tp1 = ep["actions"]                 # (T-1, D)

    ep["actions"] = q_tp1 - q_t
    return ep
    ...
    with h5py.File(output_path, "w") as h5:
        ...
        for path in tqdm(files, desc="Converting episodes"):
            ...
            try:
                ...
                ep = align_time(ep)
                ep = convert_to_delta_actions(ep)
                # ep = add_next_obs(ep) # old thingy, remove
